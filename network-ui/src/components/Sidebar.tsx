...
import type { SnaStats } from '../sna-api';
import type { Stats, Actor, TagCluster } from '../types';

const SnaStatsRow: React.FC = () => {
  const [s, setS] = useState<SnaStats | null>(null);
  useEffect(() => { snaApi.stats().then(setS).catch(() => {}); }, []);
  if (!s) return null;
  return (
    <div className="space-y-1.5 text-sm">
      <div className="flex justify-between">
        <span className="text-gray-400">Nodes</span>
        <span className="font-mono text-teal-400">{s.nodes.toLocaleString()}</span>
      </div>
      <div className="flex justify-between">
        <span className="text-gray-400">Edges</span>
        <span className="font-mono text-teal-400">{s.edges.toLocaleString()}</span>
      </div>
      <div className="flex justify-between">
        <span className="text-gray-400">Communities</span>
        <span className="font-mono text-teal-400">{s.communities}</span>
      </div>
    </div>
  );
};
    ...
    <div className="w-80 bg-gray-800 border-r border-gray-700 flex flex-col h-screen overflow-hidden">
      ...
      {stats && (
        <div className="p-4 border-b border-gray-700 flex-shrink-0 space-y-4">
          ...
          {/* SNA Network */}
          <div>
            <p className="text-gray-500 text-xs uppercase tracking-wide mb-2">SNA Network</p>
            <SnaStatsRow />
          </div>

          {/* Document Corpus */}
          <div>
            <p className="text-gray-500 text-xs uppercase tracking-wide mb-2">Document Corpus</p>
            <div className="space-y-1.5 text-sm">
              <div className="flex justify-between">
